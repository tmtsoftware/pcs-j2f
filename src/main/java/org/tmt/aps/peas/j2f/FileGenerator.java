...
public class FileGenerator {
	...
	public void generateCSourceFile(File stagingDirectory) throws Exception {
		...
		content.append("#include \"" + nameGenerator.getJNIHeaderFileName()
	...
	public String generateScriptSegment(File stagingDirectory, String fortranFileName, String currentDir, File fortranDir) throws Exception {
		...
		// copy fortran source files to staging, only if they have changed
		if (!FileUtils.contentEqualsIgnoreEOL(new File(fortranDir.getAbsolutePath() + "/" + fortranFileName), new File(stagingDirectory.getAbsolutePath() + "/" + fortranFileName), "utf-8")) {
			
			content.append("cp " + fortranDir.getAbsolutePath() + "/" + fortranFileName + " " + stagingDirectory.getAbsolutePath() + "\n");
		}
		
		// copy structures.f90 to staging, only if it has changed
		if (!FileUtils.contentEqualsIgnoreEOL(new File(currentDir + "/" + "structures.f90"), new File(stagingDirectory.getAbsolutePath() + "/" + "structures.f90"), "utf-8")) {
			// only copy if the file has changed
			content.append("cp " + currentDir + "/" + "structures.f90" + " " + stagingDirectory.getAbsolutePath() + "\n");
		}
		
		// copy logWrite.f90 to staging, only if it has changed
		if (!FileUtils.contentEqualsIgnoreEOL(new File(currentDir + "/" + "logWrite.f90"), new File(stagingDirectory.getAbsolutePath() + "/" + "logWrite.f90"), "utf-8")) {
			// only copy if the file has changed
			content.append("cp " + currentDir + "/" + "logWrite.f90" + " " + stagingDirectory.getAbsolutePath() + "\n");
		}
		
		// copy RetVal.java to staging, only if it has changed
		if (!FileUtils.contentEqualsIgnoreEOL(new File(currentDir + "/" + "RetVal.java"), new File(stagingDirectory.getAbsolutePath() + "/" + "RetVal.java"), "utf-8")) {
			// only copy if the file has changed
	...
	public static void createAndWriteFile(File dir, String filename, String content) throws Exception {
		...
		// read in contents into a String
		if (destFile.exists()) {
			String destContents = readFile(destFile.getAbsolutePath(), Charset.defaultCharset());
			
			if (content.equals(destContents)) {
				return;
			}
		}
		...
		BufferedWriter out = new BufferedWriter(new FileWriter(destFile.getAbsolutePath(), false));
		out.write(content);
		out.close();
